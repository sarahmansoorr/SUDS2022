...
temp_2002_2022 %>% filter((month_name == "Dec" | month_name == "Jan"| month_name == "Feb" | 
                             month_name == "Mar") & (year >= 2002 & year <= 2012)) %>% ggplot(aes(x=year, y=max_temp, 
                                                                                                  color=factor(month_name, level = level_order))) + 
  geom_point(alpha = 0.4) + labs(color="Month") + xlab("Year") + ylab("Maximum Temperature") + 
  theme_bw() + facet_grid(. ~ month_name)

data_new <- temp_2002_2022
data_new$month_name <- factor(data_new$month_name,
                         levels = c("Dec", "Jan", "Feb","Mar"))
data_new %>% filter((month_name == "Dec" | month_name == "Jan"| month_name == "Feb" | 
                             month_name == "Mar") & (year >= 2002 & year <= 2012)) %>% ggplot(aes(x=year, y=max_temp, 
                                                                                                  color=factor(month_name, level = level_order))) + 
  geom_point(alpha = 0.4) + labs(color="Month") + xlab("Year") + ylab("Maximum Temperature") + 
  theme_bw() + facet_grid(. ~ month_name)
...
data_new %>% filter((month_name == "Dec" | month_name == "Jan"| month_name == "Feb" | 
                       month_name == "Mar") & (year >= 2007 & year <= 2012)) %>% ggplot(aes(x=year, y=max_temp, 
                                                                                            color=factor(month_name, level = level_order))) + 
  geom_point(alpha = 0.4) + labs(color="Month") + xlab("Year") + ylab("Maximum Temperature") + 
  theme_bw() + facet_grid(. ~ month_name)
...
group.colors <- c("#4253AF", "#8BDEF2", "#D9EDF5", "#5A69A9")
data_new %>% filter((month_name == "Dec" | month_name == "Jan"| month_name == "Feb" | 
                       month_name == "Mar") & (year >= 2014 & year <= 2020)) %>% ggplot(aes(x=year, y=max_temp, 
                                                                                            color=factor(month_name, level = level_order))) + 
  geom_point(alpha = 0.4) + labs(color="Month") + xlab("Year") + ylab("Maximum Temperature") + 
  theme_bw() + facet_grid(. ~ month_name) + scale_fill_manual(values=group.colors)





level_order <- c("May", "Jun", "Jul", "Aug")
data_new1 <- temp_2002_2022
data_new1$month_name <- factor(data_new1$month_name,
                              levels = c("May", "Jun", "Jul", "Aug"))
data_new1 %>% filter((month_name == "May" | month_name == "Jun"| month_name == "Jul" | 
                       month_name == "Aug") & (year >= 2014 & year <= 2020)) %>% ggplot(aes(x=year, y=max_temp, 
                                                                                            color=factor(month_name, level = level_order))) + 
  geom_point(alpha = 0.4) + labs(color="Month") + xlab("Year") + ylab("Maximum Temperature") + 
  theme_bw() + facet_grid(. ~ month_name)
...
pollution %>% filter(pollutant == "NO") %>% ggplot(
  aes(x = factor(month_name, level = level_order), y = mean_pollution)) + geom_point(color = "#8BDEF2") +
  xlab("Month") + ylab("Mean Pollution (ppb)") + theme_bw()
...
pollution %>% filter(pollutant == "NO") %>% ggplot(
  ...
  geom_point(color = "#8BDEF2") + labs(color="Month") + xlab("Year") + ylab("Mean Pollution (ppb)") + theme_bw()
...
pollution %>% filter(pollutant == "O3") %>% ggplot(
  aes(x = factor(month_name, level = level_order), y = mean_pollution)) + geom_point(color = "#4253AF") +
  xlab("Month") + ylab("Mean Pollution (ppb)") + theme_bw()
