...
temp_2002_2022 %>% filter(month_name == "Dec" | month_name == "Jan"| month_name == "Feb" | month_name == "Mar") %>% ggplot(aes(x=year, y=max_temp, color=factor(month_name, level = level_order))) + geom_point() + labs(color="Month") + xlab("Year") + ylab("Maximum Temperature") + theme_bw()
...
temp_2002_2022 %>% filter(month_name == "Apr" | month_name == "May" | month_name == "Jun" | month_name == "Jul") %>% ggplot(aes(x=year, y=max_temp, color=factor(month_name, level = level_order))) + geom_point() + labs(color="Month") + xlab("Year") + ylab("Maximum Temperature") + theme_bw()
...
temp_2002_2022 %>% filter(month_name == "Aug" | month_name == "Sep" | month_name == "Oct" | month_name == "Nov") %>% ggplot(aes(x=year, y=max_temp, color=factor(month_name, level = level_order))) + geom_point() + labs(color="Month") + xlab("Year") + ylab("Maximum Temperature") + theme_bw()
...
pollu_temp %>% filter((pollutant != "CO" & pollutant != "SO2")) %>% ggplot(aes(max_temp, mean_pollution)) + geom_point(color = "#6DD6CD") + facet_grid(. ~ pollutant) + xlab("Maximum Temperature") +  ylab("Mean Pollution") + theme_bw()
```
...
pollu_temp %>% filter((pollutant == "CO")) %>% ggplot(aes(max_temp, mean_pollution)) + geom_point(color = "#6DD6CD") + facet_grid(. ~ pollutant) + xlab("Maximum Temperature") +  ylab("Mean Pollution") + theme_bw()
```
...
pollu_temp %>% filter((pollutant == "SO2")) %>% ggplot(aes(max_temp, mean_pollution)) + geom_point(color = "#6DD6CD") + facet_grid(. ~ pollutant) + xlab("Maximum Temperature") +  ylab("Mean Pollution") + theme_bw()
...
NO2 is usually formed through the burning of fossil fuels, transportation services, energy production and industrial processes. Nitrogen dioxide at higher concentration levels has a strong and harsh odor and can be seen over large cities as a brownish haze. NO2 can combine with H2O and form nitric acid and nitrous acid. This falls to earth through precipitation like rain, snow, and fog, and contributed to acidification and eutrophication of ecosystems.

SO2 emissions come from natural sources like volcanoes and forest fires but these emissions are small compared to those from industrial sources. Environment Canada accounted for 1 million tonnes of man-made SO2 emissions compared to 100 tonnes of natural SO2 emissions in 2022. SO2 is seen in acid rain and due to this early recognition policies were put in place that have reduced SO2 pollution in recent decades as seen in figure 14.
...
NO2 (Nitrogen Dioxide) has consequences on human health including emergency hospitalizations for respiratory conditions for adults that live in lower income communities in Toronto. Children are more vulnerable to these effects of respiratory damage. Children that have been exposed to NO2 have an increased risk of asthma, wheezing, infections in the ear, nose and throat, influenza, serious colds, and reduced lung function.
